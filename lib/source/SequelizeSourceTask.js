...
class SequelizeSourceTask extends SourceTask {
    ...
    _getTimeStampCondition() {
        let delayCondition = [];
        if (this.timestampDelaySeconds > 0) {
            delayCondition = ["AND", `${this.quoteFunction(`${this.table}.${this.timestampColumn}`)} < DATE_SUB(current_timestamp, INTERVAL ${this.timestampDelaySeconds} SECOND)`];
        }
        return [
            `((${this.quoteFunction(`${this.table}.${this.timestampColumn}`)} = :timestamp`,
            "AND",
    ...
    _getQuery() {
        let query = this._getQueryWithWhereCondition();
    ...
    _getQueryWithWhereCondition() {
        if (this.customQuery) {
            const customQuery = this.customQuery.replace(/(ORDER|GROUP BY|LIMIT|OFFSET)(.+)$/mg, '');
            return [ customQuery, (customQuery.replace(/\(.+\)/gm, '').toLowerCase().indexOf(' where ') === -1 ? 'WHERE' : 'AND') ];
        } else {
            return [ "SELECT", "*", "FROM", this.quoteFunction(this.table, true), "WHERE" ];
        }
    }
    ...
    start(properties, callback, parentConfig) {
        ...
        this.timestampDelaySeconds = timestampDelaySeconds || 0;
